...
<a href="{{ route('admin.kamar.create') }}" class="px-4 py-2 text-white bg-blue-600 rounded">
    Tambah Kamar
</a>
    ...
    <tbody>
        ...
        <tr class="border">
            ...
            <td class="flex gap-2 p-2">
                <a href="{{ route('admin.kamar.edit', $kamar->id) }}" class="px-3 py-1 text-white bg-yellow-500 rounded">Edit</a>

                <form action="{{ route('admin.kamar.destroy', $kamar->id) }}" method="POST" onsubmit="return confirm('Yakin?')">
                    @csrf
                    @method('DELETE')
                    <button class="px-3 py-1 text-white bg-red-600 rounded">Hapus</button>
                </form>
            </td>
        </tr>
        @endforeach
